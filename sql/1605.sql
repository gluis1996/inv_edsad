...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 31-05-2024 a las 23:04:52
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `InsertarEmpleado` (IN `empleado_nombre` TEXT)   BEGIN
...
DROP PROCEDURE IF EXISTS `insertar_beneficiario`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_beneficiario` (IN `p_nombre` VARCHAR(100))   BEGIN
...
DROP PROCEDURE IF EXISTS `insertar_meta`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_meta` (IN `p_nombre` VARCHAR(255))   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_oficina` (IN `p_nombres` TEXT, IN `p_idsedes` INT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_sede` (IN `p_nombres` TEXT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insertar_usuario` (IN `p_nombre` TEXT, IN `p_user` TEXT, IN `p_contraseña` TEXT)   BEGIN
...
DROP PROCEDURE IF EXISTS `obtener_oficinas_y_sedes`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `obtener_oficinas_y_sedes` ()   BEGIN
    SELECT 
        o.idoficinas,
...
DROP PROCEDURE IF EXISTS `sp_equipos_asignados_empleados`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_equipos_asignados_empleados` (IN `empleado_id` INT)   BEGIN
    select 
e.idempleado, 
...
DROP PROCEDURE IF EXISTS `sp_insertar_a_usuaria`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_a_usuaria` (IN `p_nombres` TEXT)   BEGIN
    -- Verificar si el campo nombres está vacío o nulo
    IF p_nombres IS NULL OR p_nombres = '' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El campo nombres no puede estar vacío';
    ELSE
...
--
ALTER TABLE `a_usuaria`
  MODIFY `id_area_usuaria` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=10;
...
--
ALTER TABLE `oficina`
  MODIFY `idoficinas` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=89;
