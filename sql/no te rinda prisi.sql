...
-- Tiempo de generación: 21-05-2024 a las 23:01:50
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_equipo` (IN `p_idequipos` INT, IN `p_modelo` TEXT, IN `p_descripcion` TEXT, IN `p_fecha_registro` DATE, IN `p_idmarca` INT)   BEGIN
    DECLARE v_count INT;

    -- Verificar si el campo modelo está vacío
    ...
    ELSE
        -- Comprobar si el nuevo modelo ya existe en otra fila
        SELECT COUNT(*) INTO v_count
        FROM `equipos_informaticos`.`equipos`
        WHERE `modelo` = p_modelo;

        IF v_count > 0 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El modelo ya existe';
        ELSE
            -- Si todos los campos están llenos y la fecha es válida, realizar la actualización
            UPDATE `equipos_informaticos`.`equipos` SET
				`modelo` = p_modelo,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_equipo` (IN `p_modelo` TEXT, IN `p_descripcion` TEXT, IN `p_fecha_registro` DATE, IN `p_idmarca` INT)   BEGIN
    -- Verificar si alguno de los campos está vacío
    IF p_modelo IS NULL OR p_modelo = '' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El campo modelo está vacío';
    ELSEIF p_descripcion IS NULL OR p_descripcion = '' THEN
...
DROP PROCEDURE IF EXISTS `sp_marca_registrar`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_marca_registrar` (IN `p_marca` TEXT)   BEGIN
	DECLARE v_count INT;
    -- Verificar si alguno de los campos está vacío
    IF p_marca IS NULL OR p_marca = '' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El campo marca está vacío';
    ELSE
        -- Comprobar si el modelo ya existe en la base de datos
        SELECT COUNT(*) INTO v_count
        FROM marca
        WHERE nombre = p_marca;

        IF v_count > 0 THEN
            SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'La marca ya existe';
        ELSE
            -- Si todos los campos están llenos y la fecha es válida, realizar la inserción
            INSERT INTO `equipos_informaticos`.`equipos` (`nombre`) VALUES (p_marca);
        END IF;
    END IF;
END$$
...
(53, 8350, 2, 36, 126, 2, 56, '20202020xxxxxxxx', '4 años', 'OPERATIVO', '2024-05-21', 'INSERT', '2024-05-22 03:30:43'),
(54, 8350, 2, 36, 126, 2, 56, '20202020xxxxxxxx', '4 años', 'actualizado', '2024-05-21', 'UPDATE', '2024-05-22 03:32:07'),
(55, 8350, 3, 82, 126, 2, 41, '20202020xxxxxxxx', '1', 'actualizado', '2024-05-24', 'UPDATE', '2024-05-22 03:32:51'),
(56, 8350, 2, 27, 126, 2, 32, '20202020xxxxxxxx', '7', 'eliminado', '2024-05-27', 'DELETE', '2024-05-22 03:46:35');
...
--
ALTER TABLE `detalle_asignacion`
  MODIFY `id_detalle_asignacion` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=8351;
...
--
ALTER TABLE `equipos`
  MODIFY `idequipos` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=289;
